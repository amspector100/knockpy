...
def AR1(p=30, a=1, b=1, tol=None, max_corr=1, rho=None):
    ...
    if rho is None:
        rhos = np.log(np.minimum(stats.beta.rvs(size=p, a=a, b=b), max_corr))
    else:
        if np.abs(rho) > 1:
...
def block_equi_graph(
    ...
    **kwargs,
    ...
    if mu is None:
        mu = np.zeros(p)
    X = np.dot(np.random.randn(n, p), np.linalg.cholesky(Sigma).T) + mu.reshape(1, -1)
...
class DGP:
    ...
    def __init__(self, mu=None, Sigma=None, invSigma=None, beta=None, gibbs_graph=None):
        self.mu = mu
        self.Sigma = Sigma
    ...
    def sample_data(
        ...
        **kwargs,
        ...
        if x_dist == "gibbs":
            ...
            self.X = np.dot(
                np.random.randn(n, p), np.linalg.cholesky(self.Sigma).T
            ) + self.mu.reshape(1, -1)
        elif x_dist == "ar1t":
            ...
            raise ValueError(
                "x_dist must be one of 'gaussian', 'gibbs', 'ar1t', 'blockt'"
            )
...
def create_grouping(Sigma, cutoff=0.95, **kwargs):
    ...
    link = create_correlation_tree(corr_matrix, **kwargs)
